...
In this first module, we use a simple synthetic example to build intuition for
how PB inference works and why it is needed. We will:
...
Throughout the workshop, each exercise includes a solution and short notes to
help connect the results back to the main PB inference ideas.
...
- Use the **Notes** callouts to interpret the results and connect them to the 
  main concepts.
...
Unit 00 uses synthetic examples on purpose. The goal is to make the PB inference
workflow concrete before moving to real applications in later modules.
...
Using predicted outcomes can improve efficiency, but treating them as if they
were the true observed outcomes can bias downstream inference. PB inference 
methods use a small labeled set to correct that bias while still borrowing 
information from unlabeled data and predictions.

:::

In this unit, we will:

1. **Generate** synthetic data with `ipd::simdat()`.
...
First, make sure you have the `ipd` package and a few supporting packages
installed:
...
dat_ipd <- dat |>
  filter(set_label != "training") |>
  # Keep only the columns needed for downstream PB inference
...
### Exercise 4: Naive vs. Classical Model Fitting
...
The single wrapper function `ipd()` implements multiple PB inference methods 
(e.g., **Chen & Chen**, **PostPI**, **PPI**, **PPI++**, **PSPA**) for various
...
### Exercise 5: PB Model Fitting via the Chen + Chen Estimator

Let's run one method, `chen`, proposed by 
[Gronsbell et al., 2025](https://arxiv.org/pdf/2411.19908). The **Chen + Chen** 
estimator is a PB method that combines information from:
...
Rather than treating the predicted outcomes with the same importance as the 
true outcomes, the method estimates a **data-driven weight**, $\hat{\omega}$, 
...
regression on the *labeled* data, $\hat{\gamma}_{\rm naive}^\text{all}$ is the 
estimate from the **naive** regression on *all* the data, and $\hat{\omega}$ 
reflects the amount of additional information carried by the predictions. By 
adaptively weighting the unlabeled information, the **Chen + Chen** estimator 
achieves greater precision than by using the labeled data alone, without 
...
The `ipd_model` is an `S4` object with slots for things like the coefficient, 
se, ci, coefTable, fit, formula, data_l, data_u, method, model, and intercept. 
We can extract the coefficient table using `ipd`'s `tidy` helper and compare 
with the naive and classical methods:
...
* 'Naive' often severely underestimates (biased); 'Classical' is unbiased but wide; PB methods cluster around 1.5 with better coverage than 'naive,' often similar to classical but sometimes narrower.
...
3. PB methods (Chen & Chen, PPI, PPI++, PostPI, PSPA) strike a balance: they use predictions to effectively enlarge sample size but adjust for prediction error to avoid bias.
4. Even with 'simple' linear prediction models, PB corrections can drastically improve inference on downstream regression coefficients.
